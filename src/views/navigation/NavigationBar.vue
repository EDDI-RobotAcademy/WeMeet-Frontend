<template>
  <v-app-bar :style="appBarStyle" scroll-behavior="elevate">
    ...
    <v-col cols="auto">
      <v-btn variant="plain" :ripple="false" to="/">GET MOIM</v-btn>
    </v-col>
    <v-spacer/>
    <v-spacer/>
    <v-col>
      <v-btn @click="()=>router.push('/board/qna/list')">qna</v-btn>
    </v-col>
    <v-col cols="auto">
    </v-col>
    <v-spacer></v-spacer>
    <v-col cols="auto">
      <v-btn v-if="user.hasOwnProperty('id')" @click="$router.push('/moim/form')">create</v-btn>
    </v-col>
    <v-col cols="auto">
      <v-btn v-if="!user.hasOwnProperty('id')" rounded variant="outlined" style="color: orange" @click="setSignIn">로그인
      </v-btn>
      <v-btn v-if="user.hasOwnProperty('id')" rounded variant="outlined" style="color: orange" @click="signOut">로그아웃
      </v-btn>
    </v-col>
